...
for epoch in range(num_epochs):
    ...
    if val_accuracy > best_val_accuracy:
        ...
        early_stopping_counter = 0
        torch.save(model.state_dict(), 'model_predictor_resnet50_model.pth')  # Save the best model
    else:
...
with open('model_predictor_resnet50_training.json', 'w') as f:
...
for epoch in range(num_epochs):
    ...
    if val_accuracy > best_val_accuracy:
        best_val_accuracy = val_accuracy
        early_stopping_counter = 0
        torch.save(model.state_dict(), 'model_predictor_inception_model.pth')  # Save the best model
    else:
        early_stopping_counter += 1
        if early_stopping_counter >= early_stopping_patience:
            print("Early stopping triggered.")
...
with open('model_predictor_inception_training.json', 'w') as f:
    json.dump({'train_accuracies': train_accuracies, 'val_accuracies': val_accuracies}, f)
